...
// Local fixture of the Ethernaut 2025 "NotOptimisticPortal" level.
// Vulnerabilities (same as original):
//  1. `_computeMessageSlot` loop `i < len - 1` skips the LAST element
//     -> for len==1 the hash is INDEPENDENT of receivers/datas (both acc hashes = 0)
//     -> for len==0 the hash is the SAME as len==1 (hash collision)
//  2. `_executeOperation` runs BEFORE `_verifyMessageInclusion` (CEI violation)
//  3. `onlyOwner` allows `msg.sender == address(this)` (portal self-call)
// Goal: totalSupply() > 0 (mint via executeMessage).

import { Lib_RLPReader } from "./lib/rlp/Lib_RLPReader.sol";
import { Lib_SecureMerkleTrie } from "./lib/trie/Lib_SecureMerkleTrie.sol";
