...
contract PuppetV3Pool {
    ...
    constructor(WETH _weth, DamnValuableToken _token) {
        weth = _weth;
        token = _token;
    }
    ...
    // HARDCODE: 1 WETH = 1 DVT (biar gampang test)
    function _getOracleQuote(uint128 amount) private pure returns (uint256) {
        return amount;
    }
